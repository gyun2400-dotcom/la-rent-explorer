    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        <div className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-semibold">Rent vs. Income</h2>
            <span className={`px-3 py-1 rounded-full text-sm ${darkMode ? 'bg-blue-900 text-blue-200' : 'bg-blue-100 text-blue-800'}`}>
              2024
            </span>
          </div>
          <ResponsiveContainer width="100%" height={300}>
            <ScatterChart>
              ...
              />
              <Scatter data={[...neighborhoodsData].sort((a, b) => a.income - b.income)} fill="#3B82F6" />
            </ScatterChart>
          </ResponsiveContainer>
        ...
        <div className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-semibold">Rent vs. Distance to Downtown</h2>
            <span className={`px-3 py-1 rounded-full text-sm ${darkMode ? 'bg-blue-900 text-blue-200' : 'bg-blue-100 text-blue-800'}`}>
              2024
            </span>
          </div>
...
function HeatmapPage({ neighborhoodsData, darkMode }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {/* Info Box */}
      <div className={`p-3 rounded-lg mb-6 ${darkMode ? 'bg-blue-900 border-blue-700' : 'bg-blue-50 border-blue-200'} border`}>
        <p className="text-sm">
          This heatmap is based on 2024 rental market data.
        </p>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg mb-8`}>
        ...
        <p className={`mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          Linear regression example: Relationship between square footage and monthly rent
        </p>
        <p className={`mb-6 text-sm ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>
          Multivariate model: Rent ~ Square Footage + Bedrooms + Bathrooms + Neighborhood + Parking + Furnished
        </p>
        ...
        <ResponsiveContainer width="100%" height={400}>
          <ScatterChart>
            ...
            <XAxis 
              ...
              name="Square Footage" 
              stroke={darkMode ? '#9CA3AF' : '#4B5563'}
              label={{ value: 'Square Footage (sqft)', position: 'insideBottom', offset: -5 }}
            />
            <YAxis 
              dataKey="rent" 
              name="Rent" 
              stroke={darkMode ? '#9CA3AF' : '#4B5563'}
              label={{ value: 'Monthly Rent (USD)', angle: -90, position: 'insideLeft' }}
            />
            <Tooltip 
              cursor={{ strokeDasharray: '3 3' }}
              contentStyle={{ 
              ...
              formatter={(value, name) => {
                if (name === 'Square Footage') return [`${value} sqft`, name];
                if (name === 'Rent') return [`$${value}`, name];
                return [value, name];
              }}
            />
            <Scatter 
              data={[
                { sqft: 500, rent: 1800 }, { sqft: 650, rent: 2100 }, { sqft: 750, rent: 2300 },
                { sqft: 850, rent: 2500 }, { sqft: 900, rent: 2650 }, { sqft: 1000, rent: 2800 },
                { sqft: 1100, rent: 3000 }, { sqft: 1200, rent: 3200 }, { sqft: 1350, rent: 3500 },
                { sqft: 1500, rent: 3800 }, { sqft: 600, rent: 1950 }, { sqft: 800, rent: 2400 },
                { sqft: 950, rent: 2700 }, { sqft: 1150, rent: 3100 }, { sqft: 1300, rent: 3400 }
              ]} 
              fill="#3B82F6" 
            />
            <Line 
        ...
        {/* Axis Explanation Box */}
        <div className={`mt-4 p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
          <h3 className="font-semibold mb-3">Axis Explanation</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              <span className="font-semibold">X-Axis:</span> Square Footage (sqft)
            </div>
            <div>
              <span className="font-semibold">Y-Axis:</span> Monthly Rent (USD/month)
            </div>
          </div>
        </div>
        ...
        {/* Metrics Explanation */}
        <div className={`mt-4 p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
          <h3 className="font-semibold mb-2">Metrics Explanation</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
            <div>
              <span className="font-semibold">R² Score:</span> How well the model fits the data (0-1 scale, higher is better)
            </div>
            <div>
              <span className="font-semibold">RMSE:</span> Root Mean Square Error - penalizes large errors more heavily
            </div>
            <div>
              <span className="font-semibold">MAE:</span> Mean Absolute Error - average dollar amount the model is off by
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg mb-8`}>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart 
            data={[
              { variable: 'Square Footage', importance: 85 },
              { variable: 'Bedrooms', importance: 65 },
              { variable: 'Bathrooms', importance: 55 },
              { variable: 'Neighborhood', importance: 45 },
              { variable: 'Parking', importance: 25 },
              { variable: 'Furnished', importance: 20 }
            ]} 
            layout="vertical"
          >
            <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? '#374151' : '#e5e7eb'} />
        ...
        <div className={`mt-4 p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
          <p className="text-sm">
            These values represent the relative importance of each feature in predicting rent prices. 
            A higher score means the model relies more on that variable when making predictions, while 
            a lower score indicates less influence. The scale is normalized (0–100) for easier comparison.
          </p>
        </div>
      </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <ul className="list-disc list-inside space-y-1 ml-4">
                <li>Zillow Research datasets</li>
            ...
            <div>
              ...
              <p>Data collected from January 2019 - November 2024</p>
            </div>
            <div>
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="space-y-8">
        ...
        <div className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
          ...
          <div className="space-y-3">
            <p><strong>Name:</strong> Glen Yun</p>
